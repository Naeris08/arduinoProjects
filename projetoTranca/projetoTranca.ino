...
void abrirTranca() {
  Serial.println("== Iniciando abrirTranca ==");
  Serial.println("Movendo servo para 160 graus..");
  digitalWrite(led1, HIGH);
  tranca.write(-160);
  delay(500);
  tranca.detach();
  Serial.println("Acionando buzzer..");
  tone(buzzer, 4000, 500);
  delay (500);
  Serial.println("Mantendo..");
  delay(3000);
  Serial.println("Fechando tranca...");
  tranca.attach(11);  // liga servo de novo
  tranca.write(40);
  delay(500);
   //tranca.detach(); // desliga o servo
  trancaAberta = false;
  digitalWrite(led1, LOW);
  digitalWrite(led2, LOW);
...
void resetarFluxo() {
  ...
  digitalWrite(led1, LOW);
  digitalWrite(led2, LOW);
  Serial.println("Sistema pronto. Digite a ID (4 dígitos):");
}
